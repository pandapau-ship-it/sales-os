...
- [x] Vollständigkeits-Ring über **dieselbe Registry** (`fieldImportance.ts` Voice-Felder, 11 gezählt/recommended, Scope `"voice"` in `computeCompleteness`) — keine zweite Logik
...
- [x] **Referenz-Feld-Angleichung (Migration 1/n, 21.07.2026):** Sammel-Feld `sentence_style`→4 getrennte Felder je Kanal (Tonfall/Satzbau/Wortwahl/Emoji&Format) + `hooks`→kanal-spezifisch (Post Hook-Strategien / Comment Engagement-Muster / DM+E-Mail CTA-Stil) · overview `themes`→`core_topics` (Liste). **Migr. 084 gepusht + 6/6 verifiziert** (per-Kanal-Whitelist, Listen-mit-id, alte Keys transition-safe, kein Datenverlust). `sales_approach` bleibt EIN Textfeld (`style`).
- [x] **Backend/UI-Angleichung (2/n, 22.07.2026):** db.ts-Typen gesplittet (`VoiceOverview`/`VoiceChannel`/`VoiceListItem`) · `PersonalVoicePage`/`ChannelFields` **kanal-abhängig** (Listen via `KnowledgeListField`, Texte via `KnowledgeField`, graue Icons `iconEl`, 2-Spalten-Attribut-Grid) · `fieldImportance` voice-Scope neu (weiterhin 11 gezählte, Listen/Emoji/Kanal-Spezifika optional) · `companyKnowledge.valueOf` normalisiert Text/Liste/dos_donts · i18n de/en/es (echt) · Render-/Completeness-Tests erweitert (36/36). E-Mail-Reiter + Do's&Don'ts je Kanal bleiben (bewusst). Gates grün, test-runner + auditor PASS. **STOP mit Screenshot-QA (5 Reiter) offen, noch nicht gemergt.**
- [x] **Daten-Neueingabe** der getrennten Voice-Inhalte (22.07.2026): overview + post + comment + dm über `update_voice_profile` befüllt (Listen mit `{id,text}`, Texte 1:1); email bewusst leer; alte Keys unangetastet. Remote-verifiziert.
- [x] **[D-voice-altfelder-cleanup] ERLEDIGT (22.07.2026):** alte DB-Keys `sentence_style`/`hooks`/`themes` entfernt (Migr. **085** gepusht + remote per DO-Block verifiziert: `old_gone=t` alle Kanäle+field_meta, neue Keys unverändert, alter Schreibweg lehnt `themes`/`sentence_style` ab). db.ts-Typen + i18n de/en/es bereinigt · Guard-Test `voiceCleanup.test.ts`. Gates grün, test-runner + auditor PASS. **Noch nicht gemergt.**
...
> - **Mein Unternehmen 2/3 — Personal Voice (20.07.2026): BESTANDEN** ✅ (1)+(2)+(3 für Inhalt/Schreibpfad)+(4) — Details `docs/session_uebergabe_2026-07-20.md`.
>   - **(1) Single Source ✅** — Vollständigkeit über DIESELBE Registry `fieldImportance.ts` (+20 Voice-Felder, Scope `voice`) + `computeCompleteness` (keine zweite Logik) · `update_voice_profile` = EINZIGER Schreibweg · `getMyVoiceProfile` EINZIGE Lese-Quelle · wiederverwendet `KnowledgeField`/`PanelTabs`/`SettingsCard`/`Avatar`; `SettingsCard`/`KnowledgeField` ERWEITERT (optionale Props), nicht dupliziert. Do's&Don'ts als zwei UI-Teile im SELBEN Feld `dos_donts` (keine Zweit-Spalte).
>   - **(2) Performance ✅** — audit „N+1" PASS (kein `useQuery` in `.map`); beide Queries `staleTime 60s` + org+user im Query-Key; `getMyVoiceProfile` explizite Feldliste (kein `SELECT *`); server-seitiger Shallow-Merge je Kanal → kein Read-Modify-Write-Race.
>   - **(3) Konfigurierbarkeit [[D51]] — für Inhalt/Schreibpfad ✅** — Voice-**Inhalt** liegt pro Org+User in `voice_profiles`, laufzeit-gelesen, **chat-änderbar** über `update_voice_profile`. Kein verhaltenssteuernder Schwellenwert/keine Regel im Voice-Modul (reine Nutzerinhalte). `primary_channel` = struktureller Bezeichner (System-Enum-artig, CHECK), bewusst kein UI. **Deferred (benannt, = derselbe [KONFIG-AUDIT] wie Slice 1):** die Feld-**Wichtigkeits-Rangfolge** (required/recommended/optional der 20 Voice-Felder in `fieldImportance.ts`) ist heute Code-Literal (B) — treibt nur den Vollständigkeits-Hinweis, später nach DB/chat-änderbar.
>   - **(4) Honesty ✅** — leere Voice → ehrlicher Leerzustand; Jobtitel/Firma nur wenn vorhanden (users-Spalten fehlen → korrekt unsichtbar, `// single-source-ok`); ALLE KI-Aktionen (Voice-Trainer, „So klingt das", „KI ausfüllen", Feld-KI-Knopf) sichtbar `AI_PILL_PENDING`-„Folgt" bis `lib/ai.ts` — kein erfundener Wert, keine simulierte Analyse.
>   - **Deferred:** Slice 3 (Unternehmensprofil/ICPs/Personas) · **[D-voice-admin-help]** (Owner/Admin hilft neuem Mitglied bei leerer Voice) · [KONFIG-AUDIT] Feld-Ranking (s.o.).
> - **Lifecycle-Regel-Baukasten (L-2/L-3 a–e, 23.07.2026): BESTANDEN** ✅ (1)+(2)+(3)+(4) — Details `docs/session_uebergabe_2026-07-23.md`.
>   - **(1) Single Source ✅** — EINE Filter-Sprache `src/lib/filter` (+ Deno-Spiegel `_shared/filter`) für Regeln · dynamische Listen · Analyse; `action_types` = Aktions-Registry als DATEN (kein regel-spezifisches if); `compileToPostgrest`/Dry-Run EINE Trefferzahl-Quelle (Builder-Live-Count + `getRuleMatchTargets`); `RuleMatchBanner` + `getRuleMatchTargets` geteilt Kontakte+Companies; Anker-Reads zentral in `_shared/lifecycle/anchors.ts` (kein Duplikat je Anker). EIN Schreibweg `upsert_lifecycle_rule` (chat-identisch).
>   - **(2) Performance ✅** — audit „N+1" PASS; `ruleMatchQuery` org+firedBy im Query-Key + `staleTime 30s`; Dry-Run debounced; explizite Feldlisten (kein `SELECT *`). *Deferred (benannt):* Bulk-`lifecycle_mark_fired` (ein Statement je Bündel) schließt das at-least-once-Fenster bei Mid-Loop-Crash — eigener kleiner Slice.
>   - **(3) Konfigurierbarkeit [[D51]] ✅** — Regeln SELBST sind Logik-als-Daten (Bedingung+Aktion in `lifecycle_rules`, laufzeit-gelesen, chat-fähiger Schreibweg); Schwellen der Bedingungen ziehen dieselben `settings`-Werte (churn/upsell/stagnation). **System-Enums bewusst KEIN Config** (anchor_entity, Won/Lost-Slugs, action-keys — strukturelle Bezeichner). **Deferred:** score-`*`-Crons `deleted_at` (Verschwendung, kein Verhalten) · Custom-Fields in `FILTER_SCHEMA` · CORS-Allowlist.
>   - **(4) Honesty ✅** — Deeplink-Banner deckt alle Zustände ehrlich ab (Regel weg/keine Treffer → volle Liste, nie leeres Nichts; gelöschte Treffer ausgewiesen); Zählung in Kontakt-Granularität (kein falscher „gelöscht"-Wert, Auditor-Fund C behoben); Evaluator feuert nicht auf gelöschte Anker (FUND 4/4b, `deleted_at`); `[D54]`-Strukturfehler statt stummem Scheitern.
>   - **Deferred (kein Modul-Blocker, tracked):** L-3f KI-Regel-Generierung (nach `lib/ai.ts`) · zwei offene Deploys (Push + Edge-Redeploy) · ACL-Audit aller Funktionen · Kanal-Aktionen (E-Mail/Sequenz) als coming_soon.
...
- [x] **Settings SET-3 — Team & Rechte** (19.07.2026, Migr. **076**): Settings-Shell mit Zurück-Button + vollständiger Gruppen-Nav (nur Team gebaut, Rest ausgegraut) · Mitglieder (Rolle/Status/zuletzt aktiv) · Einladen mit Dedup + Link-Kopieren · Personen-Detail mit `PERMISSIONS.map()` · alle Aktionen mit Bestätigungsdialog. Backend: `users.status` (weiches Entfernen), alle Guards, 2 Audit-Lücken geschlossen, `set_last_seen`. 18/18 live. **Offen:** Realtime user_permissions · Audit-Log-Seite (SET-6) · approval_requests (C6) · Invite-Mail ([D29]).
  - [x] **Mitglieder-Zeile-Feinschliff** (20.07.2026): Zeilen-Hover (`hover:bg-app-bg`, bestehendes Muster) · expliziter „Rechte anzeigen"-Knopf je Zeile (öffnet Rechte-Panel) · Rollen-Badge vor den Status verschoben, als `StatusBadge` — **nur Owner** dezent farbig (`tone="info"`), andere neutral/grau (`muted`); keine Rolle-/Status-Überschriften. Render-Test (3). i18n `viewPermissions` de/en/es.
