...
- [~] **Companies-Detailseite K-4b-2 (offen)** — Deals-Tab (`getDealsByCompany` + `DealsListe`) · Aktivität-Tab (aggregierter Touchpoint-Zeitstrahl aller Firmen-Kontakte, ein Query) · Notizen-Tab (`notes.company_id` + `NotizenListe`). *(Companies = eigenständiger Nav-Screen mit Sidebar-Icon.)*
