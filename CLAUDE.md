...
| `KnowledgeField` | Feld im Bereich „Mein Unternehmen" (Produkte & Preise; Slice 2/3 nutzen es weiter): Label + Inline-Edit + **zwei Aktions-Symbole — Stift (speichert über die zentrale Update-Funktion des Aufrufers) und KI-Knopf (Vorschlag für GENAU DIESES Feld; heute ausgegraut + „Folgt", da `lib/ai.ts` fehlt)**. Kein Analog: `DetailField` hat keinen zweiten Aktions-Slot und ist CRM-Panel-Semantik. Leer ist gültig (keine Pflichtfelder in diesem Bereich) |
