...
| `RuleRow` | Klartext-Regel-Zeile (SET-4 Fundament): Satz mit EINEM anklickbaren Wert-Chip → shadcn-Popover (Zahl-Eingabe + Min/Max, Doppelvalidierung Client + `update_settings`), save-on-commit über `onSave`. SET-4a-Erweiterung: `recommended`-Prop (Reset-pro-Regel, hover-gated) + `why`-Slot (`<WhyPopover>`). |
...
| `WeightEditor` | Gewichte-Editor additiver Score-Signale (Churn/Upsell, SET-4a): je Signal `Switch` (An/Aus) + `Slider` 0–`WEIGHT_MAX` + Zahl. EHRLICHE relative-Wichtigkeit-Darstellung (Anteils-Balken; Score wird 0–100 normalisiert — KEIN „Fortschritt bis Schwelle"/„über 100"; Schwelle + read-only Bänder rendert der Aufrufer separat). Nicht messbare/externe Signale (`InactiveSignal`, Grund `not_measured`/`integration`) ausgegraut, nicht editierbar. Schreiben via `onWeightChange`/`onActiveToggle` (→ `updateSettings`). |
