...
| `CompactContactRow` | Schlanke Kontakt-Listenzeile (Avatar · Name/Subzeile · ICP · „vor X Tagen" · Status-Badge · Routing · Pfeil) OHNE Tabellen-Gerüst — für kompakte In-Panel-Listen (erste Nutzung: Companies-Detail → Kontakte-Tab). Prop-driven, Werte aus dem Mapper (`contactToKontakteRow`). **K-FS1-Basis:** der spätere Hunter-Dedup (LeadListRow/HunterCard) soll DIESE Zeile übernehmen, nicht eine weitere Variante bauen |
...
| `KnowledgeField` | Feld im Bereich „Mein Unternehmen" (Produkte & Preise · Personal Voice · Unternehmensprofil). **Verbindliches Muster des Bereichs: Label oben, darunter IMMER ein sichtbares graues Eingabefeld (`FIELD`-Kanon), gespeichert beim Verlassen — kein Stift, kein Read-Mode.** Diese Seiten werden ausgefüllt, nicht gelesen; Read-Mode + Inline-Edit bleibt den CRM-Panels (`DetailField`) vorbehalten. Dazu **ein KI-Knopf je Feld** im Pill-Kanon `AI_PILL_PENDING` (Teal-Tint wie die Statistik-Pills; Vorschlag für GENAU DIESES Feld; heute nicht bedienbar + „Folgt", da `lib/ai.ts` fehlt) — er schreibt später über denselben zentralen Weg wie die Tastatureingabe. Leer ist gültig (keine Pflichtfelder in diesem Bereich). Optionaler `icon`-Prop = kleines Icon links vom Label (z.B. ✓/⚠ bei „Das machst du immer/nie" in Personal Voice) |
| `KnowledgeListField` | Generisches **wachsendes Listen-Feld** für „Mein Unternehmen" (jsonb-Arrays mit `{id,...}`-Einträgen). Nutzt `KnowledgeField` je Sub-Feld (`showAi={false}`) → gleicher `FIELD`-Kanon, Save-on-Blur. `fields`-Config (1..n Sub-Felder): **Einzelfeld** → flex-Zeile `[Feld][Entfernen]`, **Mehrfeld** → umrandete Karte (transparenter BG, graue Felder als Kontrast, Entfernen absolut oben rechts). Hinzufügen legt Eintrag mit `crypto.randomUUID()` + leeren Sub-Feldern an; **Entfernen direkt (kein Bestätigungsdialog** — nur ungespeichertes Formularfeld). `onChange` gibt die VOLLE Liste zurück (Consumer schreibt via einem Pfad, z.B. `updateOrgProfile`). Erste Nutzung: `CompanyProfilePage` (USPs · Probleme · Ergebnisse · Angebote · Wettbewerber direkt/adjacent). Regel-A-Vorbild war `DetailPhoneList`, aber dessen Read-Mode passt nicht → eigener Baustein. |
| `SettingsCard` | Karten-Sektion für Settings-Seiten (Ebene-1-Karte auf Seiten-BG): Titel (`typo-card-title`) · Beschreibung (`typo-subline`) · dezenter Header-Slot **„Gespeichert ✓"** (Prop `saved`: `'saving'`/`'saved'`/`null`, Token `signal-success`). Kein Analog: `DetailSection` hat weder Beschreibung noch Save-Slot. Erste Nutzung: SET-2 „Persönlich"-UI (`MyProfileTab`/`AppearanceTab`/`SecurityTab`). Echter Save-Zustand über `useSaveState`-Hook (kein Fake-Delay). Optional: `headerAction` (Aktions-Slot oben rechts, z.B. „KI ausfüllen"-Pill, links vom Save-Zustand) · `className` (via `cn()`, z.B. `mb-0 h-full` im Grid — Personal Voice legt Voice-Karte + Trainer 6/6 nebeneinander). |
| `RuleRow` | Klartext-Regel-Zeile (SET-4 Fundament): Satz mit EINEM anklickbaren Wert (`ValueChip`, Single Source des Chips), `recommended`-Reset (hover-gated) + `why`-Slot (`<WhyPopover>`). `valueFirst` stellt den Chip an den Zeilenanfang (linksbündige Werte-Spalte, Gruppe „Signale & ICP"); `tone` (neutral/urgent/success) färbt den Chip. |
| `ValueChip` | EIN geteilter, hervorgehobener „anklickbarer Wert" (SET-4a): Box mit Zahl (+Einheit dezent), einheitlich über alle Regeln-Gruppen. Editierbar → shadcn-Popover (Zahl-Eingabe + Min/Max, Doppelvalidierung Client + `update_settings`), Fokus = eine saubere Rahmenlinie (kein Ring). `tone` = Akzent (neutral/`urgent`/`success`). Extrahiert aus `RuleRow` → auch von `HeatThresholdTile` genutzt. Schreiben via `onSave` (→ `updateSettings`). |
| `HeatThresholdTile` | Farbige Heat-Stufen-Kachel (SET-4a, Gruppe 1): getöntes Label + Lucide-Icon & Status-Wort in der Heat-Farbe (`HEAT_STATUS[key].color`-Token, kein Hex) + „Gilt als … bis" + editierbarer Tage-`ValueChip`; letzte Stufe (Gone) `readOnlyText` statt Chip. Prop-driven, dark-mode automatisch. |
| `WhyPopover` | „Warum?"-Erklärung eines Regel-/Score-Werts (C21, systemweiter Standard): `{title, description, usedBy}` (usedBy = ECHTE Funktion, z.B. `score-churn-risk` — keine erfundenen Namen). HelpCircle-Trigger + Popover, tokens-only, i18n. |
| `WeightEditor` | Gewichte-Editor additiver Score-Signale (Churn/Upsell, SET-4a): je Signal `Switch` (An/Aus) + `Slider` 0–`WEIGHT_MAX` + Zahl. EHRLICHE relative-Wichtigkeit-Darstellung (Anteils-Balken; Score wird 0–100 normalisiert — KEIN „Fortschritt bis Schwelle"/„über 100"; Schwelle + read-only Bänder rendert der Aufrufer separat). `tone` (`urgent`/`success`) färbt Switch+Slider+Balken über Token-Override `--color-primary` (kein Hex). **Slider-Fix:** Ziehen ändert lokalen State (`onValueChange`), Persistenz nur bei `onValueCommit` (ein Write pro Drag, kein Reset-Sturm). Nicht messbare/externe Signale (`InactiveSignal`, Grund `not_measured`/`integration`) ausgegraut, nicht editierbar. Schreiben via `onWeightChange`/`onActiveToggle` (→ `updateSettings`). |
| `ConditionRow` | Bedingungs-Zeile des Regel-Builders (Lifecycle L-3b): Feld · Operator · Wert, **Wert-Eingabe passt sich Typ×Operator an** (enum+in/not_in → `EnumMultiSelect` string[] · text/number+in/not_in & tags+has_any → `ChipInput` Array · enum+eq → Einzel-Select · sonst Input). `markInvalid` markiert Feld/Operator/Wert rot. Prop-driven (`entity`/`rule`/`onChange`/`onRemove`), auch für dynamische Listen nutzbar. |
| `ConditionBuilder` | Generischer AND/OR-Bedingungs-Baum (Lifecycle L-3b): Gruppen je Datenart (Option-B), **innerhalb** Gruppe UND/ODER schaltbar, **EIN** Zwischen-Gruppen-Schalter + sichtbarer Option-B-Hinweis (`crossEntityHint`), Leerzustand, Datenart-Hinzufügen. `markInvalid`-Durchreichung an `ConditionRow`. Prop-driven (value/onChange) → wiederverwendbar (Lifecycle-Regeln + dynamische Listen). |
| `EnumMultiSelect` | Popover-Mehrfachauswahl über feste Enum-Werte (Lifecycle L-3b/FUND 2): speichert `string[]`, `rounded-[6px]`-Check-Indikator. Für `in`/`not_in` auf Enum-Feldern (Heat/Status …). |
| `ChipInput` | Chip-Token-Eingabe (Lifecycle L-3b/FUND 2): Enter/Komma fügt hinzu, Backspace/✕ entfernt; `numeric`-Option (Zahlen-Spinner ausgeblendet). Für freie Listen (text/number `in`/`not_in`, tags `has_any`). |
| `LiveMatchCount` | Live-Trefferzahl des Regel-Builders (Lifecycle L-3b): debounced Dry-Run über TanStack Query (kein setState-in-effect), Zustände Idle/Loading/Error/Count. |
| `RuleMatchBanner` | Deeplink-Kopfzeile über Kontakte-/Companies-Liste (Lifecycle L-3e): Sprung aus gefeuerter Regel-Benachrichtigung (`?firedBy=<ruleId>`). Geteilt (Single Source, beide Screens), tokens-only, i18n. 5 ehrliche Zustände: laden · Ladefehler · Regel weg (→ volle Liste) · keine Treffer mehr (→ volle Liste, nie leeres Nichts) · Treffer (+ Hinweis „N inzwischen gelöscht"). `onClear` entfernt `?firedBy`. Speist sich aus `db.getRuleMatchTargets` (org-gescoped, Anker→Kontakt/Firma, `deleted_at`-gefiltert, Zählung in Kontakt-Granularität). |
...
| `PersonalVoicePage` | Settings → Mein Unternehmen → **Personal Voice** (Slice 2/3, Migr. 078/079): eigene Schreibstimme **pro User** (`visibility:'self'`), getrennt von `contacts.personality_profile`. 5 Kanäle **overview + post/comment/dm/email** (`email` bewusste 5. Ergänzung) via `PanelTabs`; je Feld `KnowledgeField`. Kanal-Felder Schreibstil · Aufmacher · **Do's&Don'ts als ZWEI benannte Teile im SELBEN Feld `dos_donts`** (`{always,never}`, keine Migration) · Beispiele ganz unten. Voice-Karte + **AI-Voice-Trainer nebeneinander (6/6)**; der Trainer ist eine **bewusste Einzelfall-Ausnahme** (Original-Dunkel-Optik türkis via Tokens `--voice-trainer-*` in `index.css`, KEIN Hex im JSX) mit zwei echten `ui/button` (size sm). ALLE KI-Aktionen `AI_PILL_PENDING`-„Folgt" bis `lib/ai.ts`. Vollständigkeits-Ring über DIESELBE Registry (`fieldImportance.ts` Scope `voice`). EINZIGER Schreibweg `update_voice_profile`. |
| `RuleOverview` (`features/settings/lifecycle/`) | Settings → **Automatik-Regeln** (Lifecycle L-3c): Regel-Liste, Limit-Anzeige „X/Y Regeln" + `atLimit`-Banner, Vorlagen-Galerie (`templates.ts`, 5 echte Vorlagen), `automation.manage`-Gate auf Seiten-Ebene, Löschen-`alert-dialog`, Toggle/Delete via `upsert/deleteLifecycleRule`-Mutations, Zustände Laden/Fehler/Leer/Liste. `onNew`/`onEdit`/`onTemplate`-Callbacks → Editor. Optionaler `org`-Prop (Default Demo-Org). |
| `RuleCard` (`features/settings/lifecycle/`) | Eine Regel-Zeile (Lifecycle L-3c): Datenart-Chip (Entity-Farb-Token), Klartext-Satz (`summary.ts`), „zuletzt gefeuert für X", Aktiv-Schalter, Edit/Delete hover, mobil stapelnd (Reflow < 640px). |
| `RuleEditor` (`features/settings/lifecycle/`) | 3-Schritt-Regel-Editor (Lifecycle L-3d): Anker → Bedingungen (`ConditionBuilder`, `markInvalid={showErrors}`) → Aktion (`action_types`-Registry, coming_soon ausgegraut, dynamische Listen bei `add_to_list` ausgegraut + Begründung). Speichern gated auf gültige Aktion UND Bedingungen; „Nicht gespeichert"-Banner + Grund in Alltagssprache. EIN Schreibweg `upsertLifecycleRule` (`expectedUpdatedAt`-Guard → `stale_write`). |
...
- **`settings.pipeline_stages`** — Stages als JSONB (top-level), nicht hardcodiert. `deals.stage` speichert den **Slug** (`backlog`…`gewonnen`/`verloren`), nicht den Anzeigenamen — Name kommt aus `settings.pipeline_stages[].name`
  - **SYSTEM-INVARIANTE — Won/Lost-Slugs (`gewonnen`/`verloren`) sind ein System-Enum, bewusst KEIN [D51]-Config-Wert:** Die terminale Stage-Identität ist ein struktureller Bezeichner (Teil des `DealStage`-Typs `src/types/hunter.ts` + Write-Pfad `updateDealWon`/`updateDealLost`). Won/Lost wird **ausschließlich am Slug** erkannt (nie an `probability`). **Eine Quelle:** Frontend `hunterMappers` (`WON_STAGE_SLUG`/`LOST_STAGE_SLUG`/`isTerminalStage`) ↔ Edge `supabase/functions/_shared/terminalStages.ts` (Spiegel; Deno kann die TS-Lib nicht importieren) — ändert sich der Enum, **beide gleich halten**. **Org-konfigurierbar (C) sind NUR die aktiven Stufen** (Name/Reihenfolge/Anzahl/stagnation_days/probability). Eine künftige Stage-Management-UI darf Gewonnen/Verloren NICHT löschen/umbenennen (geschützte Anker).
- **`org_icps` / `org_personas`** (Migr. 081, „Mein Unternehmen" Slice 3b) — Zielgruppen + Personen je Zielgruppe (1:N, `org_personas.icp_id` CASCADE), nach dem `products`-Muster (eigene Tabellen + create/update/delete-RPCs, weiches Löschen `is_active`). `org_personas.job_titles` speist `match_persona` (AI SDR).
  - **SYSTEM-INVARIANTE — `org_icps.fit_level` (`high`/`medium`/`low`) und `org_personas.buying_role` (`decision_maker`/`influencer`/`champion`/`end_user`/`blocker`) sind feste System-Enums, bewusst KEIN [D51]-Config-Wert** (analog Won/Lost-Slugs · `contact_status`): strukturelle Bezeichner, an die Auswertungs-/Matching-Logik andockt — **nicht org-konfigurierbar, kein settings-Wert**. Quelle der zulässigen Werte ist der `CHECK` in Migr. 081; beide sind **nullable** (leer = noch nicht bewertet/zugeordnet, Honesty). Umbenennen/erweitern = Code+Migration-Änderung, nie Org-Config.
...
> **Katalog-Umfang (Stand SET-2, 19.07.2026):** Die Matrix oben ist der **Ziel-Endzustand**. Der
> **`permission_catalog` enthält NUR die heute gebauten Rechte** — `team.invite` · `records.delete` ·
> `records.merge` · **`settings.manage`** (SET-2, Migr. 073). Alle übrigen Rechte (rules.edit, campaigns.manage,
> pipeline.manage, billing.*, export.all, trash.purge, audit.view, branding.manage, lists.share …) werden
> **MIT ihrem Modul** ergänzt (Dauerregel „Rechte-Check-Pflicht"). Zukunfts-Registry: PROGRESS.md.
