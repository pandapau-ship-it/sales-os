...
> **Signals-Tab (Slice 3):** 1:1 Hunter-Signals-Muster — `LinkedinSignalCard` additiv erweitert (`channelTone` info/urgent/success · `channelLabel` Klartext · `statusBadge`-Passthrough · `actionLabel`); **Hunter unverändert** (Defaults). Neue geteilte `SubscriptionBadge`-Komponente. 3 Mock-Signale (PayGuard=LinkedIn/blau · HiringMate=Churn/rot · Logistify=Upsell/grün), SUBSCRIPTION statt STAGE, Bulk-Auswahl-Leiste wie Hunter, CTA = Platzhalter-Toast ([D34]). Kein DB-Wiring. **[D33]/[D34]** dokumentiert (Farmer Info-/Action-Panel).
