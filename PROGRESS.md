...
- **Ist-Stand Hunter (Diagnose 30.06.2026 → Slice 1 gefixt):** C ✅ = Heat-Grenzen (`heat_status`), Stagnationsschwellen pro Stage (`pipeline_stages[].stagnation_days`), Hunter-Priority-Gewichte (`hunter_priority_weights`, 045). **Slice 1 geschlossen:** stummer Fallback (priority-Gewichte/Stagnation `?? default` bei `getSettings()===null`) → ReferenceScreens Drei-Zustands-Gate (eine `settingsQuery`, alles daraus abgeleitet); „Neu in Pipeline"-Fenster (7/30) → `settings.thresholds.timing_windows.new_pipeline_short_days/_long_days` (Migr. 055). **Offen Slice 2:** Terminal-Stage-Slugs („gewonnen"/„verloren" hartcodiert in score-deal-health + score-upsell + hunterMappers) → `type`-Flag an `pipeline_stages`. **Deferred (nicht Verstoß / noch nicht gebaut):** ICP-Berechnung (Spalte nie befüllt — wird später selbst gebaut), `icp_score_threshold:65` BEHALTEN (Vorab-Einstellung, Konsument folgt), ICP-Bänder vereinheitlichen (Donut 75/50 + priority 80/60 → ein Satz), Signal→Aktion-Resolver-Konfig, Signal-Routing-Regel (`routed_to`), Deal-Health-Kompositum, AI-SDR-Gating.
...
- **NÄCHSTER Slice (direkt danach, eigener Branch):** FarmerReference auf dieselbe **Single-`settingsQuery` + Drei-Zustands-Guard** umstellen wie HunterReference (Einheitsgebot) — prüfen ob Farmer noch Mehrfach-getSettings nutzt, falls ja angleichen. Reines Refactoring, Happy-Path unverändert.
