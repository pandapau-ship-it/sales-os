...
stage               text DEFAULT 'backlog'
                    -- KANONISCHE Default-Stages (frei konfigurierbar pro Organization
                    --   aus settings/pipeline_stages — nie hardcodiert):
                    --   backlog | demo_vereinbart | followup_offen | onboarding_offen |
                    --   free_trial | gewonnen
                    -- Terminal (kein Stagnations-Timer): gewonnen | verloren
                    --   (verloren erfordert lost_reason)
value               bigint                        -- in Cent
currency            text DEFAULT 'EUR'
probability         int                           -- Win-Probability % (0-100); erbt Stage-Default
                                                  --   aus settings.pipeline_stages, pro Deal überschreibbar
                                                  -- Gewichteter Pipeline-Wert = value × probability
...
thresholds          jsonb DEFAULT '{}'
                    -- {heat_status_days, churn_risk_weights, upsell_weights, ...}
pipeline_stages     jsonb DEFAULT '[]'
                    -- kanonische Stages, frei konfigurierbar pro Org (nie hardcodiert):
                    -- [{name, order, stagnation_days, probability}]
                    --   probability = Win-Probability % (Stage-Default für deals.probability)
