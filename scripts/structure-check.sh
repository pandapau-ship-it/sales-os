...
WRONG=$(find src/components/shared -maxdepth 1 -name "*.tsx" | grep -v -E "(Avatar|LinkedinIcon|Toast|EmptyState|CommandPalette|ICPDonut|BrandLogo|BrandIcons|CommunicationChain|CustomerDrawer|Badge|TooltipLayer|DataTableCard|ColumnConfigPopover|TableSearch|Stepper)")
