...
export default function ActionComposer({
  ...
  return (
    <div className="flex items-end gap-2 bg-app-bg border border-border rounded-[14px] px-3 py-2 focus-within:border-[var(--sherloq-primary)] focus-within:bg-app-surface transition-all">
      ...
      />
      <button onClick={send} aria-label="Senden" className="w-9 h-9 rounded-full text-on-accent flex items-center justify-center shrink-0 hover:scale-105 transition-transform cursor-pointer shadow-sm" style={{ background: "var(--sherloq-gradient)" }}>
