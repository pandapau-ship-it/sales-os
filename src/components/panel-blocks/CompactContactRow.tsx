 ...
 * ICP-Ring · „vor X Tagen" · Status-Badge · Routing · Pfeil. Optik = Kontakte-Tabellenzeile, aber
 * OHNE Tabellen-Gerüst (keine Spaltenköpfe/Sortierung/Konfig) — für kompakte In-Panel-Listen
 * (erste Nutzung: Companies-Detail → Kontakte-Tab). Prop-driven; die Werte kommen aus dem
 * aufrufenden Mapper (Single Source, z.B. `contactToKontakteRow`).
 *
...
export default function CompactContactRow({
  name, jobTitle, company, avatarUrl, icpScore, contactStatus, lastContactedAt, routing, openLabel, onOpen, onNavigate, onPrefetch,
  ...
  return (
    <div {...bind(onPrefetch)}
      ...
      {icpScore != null && <ICPDonut score={icpScore} />}
      {days != null && days >= 1 && <span className="typo-field-value text-text-body whitespace-nowrap">{t("kontakte.daysAgo", { count: days })}</span>}
      {contactStatus && STATUS_TONE[contactStatus] && <StatusBadge label={t(`kontakte.status.${contactStatus}`)} tone={STATUS_TONE[contactStatus]} />}
      {routing && <RoutingChip routing={routing} onNavigate={onNavigate} />}
