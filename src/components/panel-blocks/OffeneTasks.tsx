...
export default function OffeneTasks({
  onAdd, onOpenTasks, onEditTask, onToast,
}: { onAdd?: () => void; onOpenTasks?: () => void; onEditTask?: (id: string) => void; onToast?: (msg: string) => void }) {
  const stop = (e: React.MouseEvent) => e.stopPropagation();

  const Actions = ({ taskId }: { taskId: string }) => (
    <div className={`flex items-center gap-1 shrink-0 ${HOVER_ACTIONS}`}>
      <button onClick={(e) => { stop(e); onEditTask?.(taskId); }} aria-label="Bearbeiten" data-tip="Bearbeiten" className="w-7 h-7 rounded-full flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-app-surface transition-colors cursor-pointer">
        <Pencil className="w-3.5 h-3.5" />
      </button>
      <button onClick={(e) => { stop(e); onToast?.("Task gelöscht"); }} aria-label="Löschen" data-tip="Löschen" className="w-7 h-7 rounded-full flex items-center justify-center text-text-muted hover:text-[var(--signal-urgent-text)] hover:bg-[var(--signal-urgent-bg)] transition-colors cursor-pointer">
        <Trash2 className="w-3.5 h-3.5" />
      </button>
      <button onClick={(e) => { stop(e); onToast?.("Task erledigt ✓"); }} aria-label="Erledigt" data-tip="Erledigt" className="w-7 h-7 rounded-full flex items-center justify-center text-text-muted hover:text-[var(--signal-success-text)] hover:bg-[var(--signal-success-bg)] transition-colors cursor-pointer">
