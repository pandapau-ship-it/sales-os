...
export default function CommunicationChain({ personId, onSelectCommunication }: CommunicationChainProps) {
  ...
  return (
    <div className="w-full mt-4 bg-app-bg rounded-[24px] p-6 border border-border">
      <div className="text-[11px] font-bold font-mono text-text-muted uppercase tracking-wider mb-10 text-center">
        ...
        {chain.map((tp, idx) => {
          return (
            ...
            >
              ...
              {/* Tooltip */}
              {hoveredId === tp.id && (
                <div className="absolute bottom-16 left-1/2 -translate-x-1/2 w-48 bg-app-surface border border-border shadow-[0_10px_30px_rgb(0,0,0,0.1)] rounded-card p-3 z-50 animate-fade-in text-left">
                   <div className="flex justify-between items-center mb-2">
                     <span className="text-[11px] font-bold text-sherloq-primary font-mono">{tp.tooltip.channel}</span>
                     <span className="text-[10px] text-text-muted">{tp.tooltip.date}</span>
                   </div>
                   <div className="text-[12px] text-text-body mb-1">
                   ...
                   {/* Tooltip triangle */}
                   <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3 h-3 bg-app-surface border-b border-r border-border rotate-45" />
                </div>
              )}
            </div>
          )
