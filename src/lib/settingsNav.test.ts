...
describe("Vollständige Settings-Navigation (SET-3, Bauplan Abschnitt 1)", () => {
  ...
  it("gebaut sind 'team' + 'unternehmensprofil' + 'personal-voice' + 'product-pricing' — Rest ausgegraut ('Folgt')", () => {
    const builtInNav = SETTINGS_PAGES.filter((p) => p.group !== "personal" && p.built).map((p) => p.key);
    expect(builtInNav).toEqual(["team", "unternehmensprofil", "personal-voice", "product-pricing"]);
  });
